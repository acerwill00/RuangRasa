...
<div class="min-h-screen bg-slate-50 py-12 px-4">
    <div class="max-w-3xl mx-auto">
        ...
        {{-- Page Header --}}
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-text-main">{{ __('My Profile') }}</h1>
            <p class="text-slate-500 mt-1">{{ __('Manage your personal information and profile photo.') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" 
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-8">
                    ...
                    <div class="flex flex-col gap-3 text-center sm:text-left">
                        ...
                        <p class="text-xs text-slate-400">{{ __('JPG, PNG or WebP. Max 2MB.') }}</p>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
                ...
                <div class="space-y-5">
                    {{-- Full Name --}}
                    <div>
                        <label for="name" class="block text-sm font-semibold text-text-main mb-2">{{ __('Full Name') }}</label>
                    ...
                    <div>
                        <label for="email" class="block text-sm font-semibold text-text-main mb-2">
                            {{ __('Email Address') }}
                            <span class="ml-2 text-xs font-normal text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{{ __('Cannot be changed') }}</span>
                        </label>
                    ...
                    {{-- Phone --}}
                    <div>
                        <label for="phone" class="block text-sm font-semibold text-text-main mb-2">{{ __('Phone Number') }} <span class="text-slate-400 font-normal">{{ __('(optional)') }}</span></label>
                        <div class="relative">
