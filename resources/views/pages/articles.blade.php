...
<div x-data="{ filter: 'all' }">
    ...
    <section class="max-w-7xl mx-auto px-6 pt-16 pb-12">
        <div class="text-center max-w-2xl mx-auto mb-10">
            <h1 class="text-4xl md:text-5xl font-bold mb-6 text-text-main">{{ __('Mental Health Resources') }}</h1>
            <p class="opacity-80 text-lg">{{ __('Insights, tips, and guidance written by professionals to help you navigate life\'s challenges safely.') }}</p>
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 pb-24 min-h-[500px]">
                ...
                <div class="p-7 flex flex-col flex-grow">
                    ...
                    <div class="pt-4 border-t border-secondary mt-auto flex justify-between items-center">
                        <a href="/article/{{ $article['slug'] }}" class="inline-flex items-center gap-2 font-bold text-text-main hover:text-cta transition-colors text-sm">
                            {{ __('Read Article') }} <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                        </a>
                    </div>
                </div>
            </article>
        ...
        <div class="mt-20 bg-primary/20 rounded-[2.5rem] p-8 md:p-14 flex flex-col lg:flex-row items-center justify-between gap-10 border border-primary/40 relative overflow-hidden shadow-soft">
            ...
            <div class="relative z-10 max-w-2xl text-center lg:text-left">
                <span class="block text-cta font-bold tracking-widest uppercase text-sm mb-3">{{ __('Professional Support') }}</span>
                <h3 class="text-3xl md:text-4xl font-bold mb-4 text-text-main">{{ __('Talk to a psychologist about these topics.') }}</h3>
                <p class="opacity-80 text-lg leading-relaxed text-text-main">{{ __('Reading articles is a great start, but talking works. Our empathetic professionals can provide personalized, actionable strategies tailored to your exact situation in a safe environment.') }}</p>
            </div>
