...
<section class="pt-16 pb-12 bg-slate-50 border-b border-primary/10">
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center max-w-2xl mx-auto mb-10 text-text-main">
            <h1 class="text-4xl md:text-5xl font-bold mb-4 leading-tight" x-show="activeSection === 'all'">{!! __('Find the Right <span class="text-cta">Support</span> for You') !!}</h1>
            <h1 class="text-4xl md:text-5xl font-bold mb-4 leading-tight" x-show="activeSection === 'today'" style="display: none;">{!! __('Available <span class="text-cta">Today</span>') !!}</h1>
            <h1 class="text-4xl md:text-5xl font-bold mb-4 leading-tight" x-show="activeSection === 'evening'" style="display: none;">{!! __('Evening & <span class="text-cta">Weekend</span>') !!}</h1>
            <h1 class="text-4xl md:text-5xl font-bold mb-4 leading-tight" x-show="activeSection === 'highly_recommended'" style="display: none;">{!! __('Highly <span class="text-cta">Recommended</span>') !!}</h1>
            
            <p class="opacity-80 text-lg mb-8">{{ __('Browse our verified clinical psychologists. Choose someone who aligns with your current needs in a safe, warm environment.') }}</p>
        ...
        <div class="flex flex-col lg:flex-row items-center justify-between gap-4 bg-white p-3 rounded-[2rem] shadow-sm border border-secondary lg:rounded-full relative z-10">
            ...
            <div class="w-full lg:w-auto px-2 pr-4 lg:pr-2 flex items-center gap-2">
                ...
                <select x-model="topic" class="w-full lg:w-auto pl-5 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-full text-sm font-bold text-slate-600 hover:border-cta focus:border-cta outline-none transition-colors appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20width%3D%2220%22%20height%3D%2220%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%2020%2020%22%20fill%3D%22%2364748B%22%3E%3Cpath%20d%3D%22M5.293%207.293a1%201%200%20011.414%200L10%2010.586l3.293-3.293a1%201%200%20111.414%201.414l-4%204a1%201%200%2001-1.414%200l-4-4a1%201%200%20010-1.414z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.2rem_1.2rem] bg-no-repeat bg-[right_0.75rem_center]">
                    <option value="">{{ __('Filter by Topic') }}</option>
    ...
    <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="relative mb-20">
                <div class="flex justify-between items-end mb-8 pl-2">
                    <div>
                        ...
                        <p class="text-[15px] opacity-60 mt-1.5 font-medium">{{ __('Get immediate support from verified professionals.') }}</p>
                    </div>
                    <button @click.prevent="activeSection = 'today'" class="text-sm font-bold text-cta hover:text-cta-hover transition-colors hidden sm:flex items-center gap-1 border-b-2 border-cta/30 pb-0.5">
                        {{ __('View All') }} <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </button>
                </div>
            ...
            <div class="relative mb-20">
                <div class="flex justify-between items-end mb-8 pl-2">
                    <div>
                        ...
                        <p class="text-[15px] opacity-60 mt-1.5 font-medium">{{ __('Professionals offering off-hours scheduling for busy workers.') }}</p>
                    </div>
            ...
            <div class="relative">
                <div class="flex justify-between items-end mb-8 pl-2">
                    <div>
                        ...
                        <p class="text-[15px] opacity-60 mt-1.5 font-medium">{{ __('Top-rated counselors according to patient feedback.') }}</p>
                    </div>
        ...
        <div x-show="isFiltering" style="display: none;" x-transition>
            ...
            <div class="mb-8 flex flex-wrap items-center gap-2">
                <span class="text-sm font-semibold text-text-main/70 mr-2">{{ __('Applied Filters:') }}</span>
                ...
                <span x-show="activeSection !== 'all'" class="inline-flex items-center gap-1.5 px-3 py-1 bg-primary/20 border border-primary/30 rounded-lg text-sm font-medium text-cta">
                    {{ __('Category:') }} <b x-text="activeSection.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')"></b>
                ...
                <button @click="clearFilters()" class="ml-2 text-xs font-bold text-cta hover:text-red-500 transition-colors underline decoration-cta/30 hover:decoration-red-500/30 underline-offset-2">{{ __('Clear All') }}</button>
            </div>
            ...
            <div x-show="Object.keys(psychologistsData).filter(id => matchesFilter(id)).length === 0" class="text-center py-20 bg-slate-50 rounded-[2rem] border border-slate-200" style="display: none;">
                ...
                <h3 class="text-xl font-bold text-text-main mb-2">{{ __('No Psychologists Found') }}</h3>
                <p class="opacity-70 max-w-md mx-auto mb-6">{{ __('We couldn\'t find anyone matching your exact specific criteria. Try adjusting your search query or filters.') }}</p>
                <button @click="clearFilters()" class="text-cta hover:text-cta-hover font-bold border-b-2 border-cta/30 hover:border-cta transition-all">{{ __('Clear All Filters') }}</button>
            </div>
