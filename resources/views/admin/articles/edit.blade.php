...
<div class="bg-secondary/10 py-10 min-h-[calc(100vh-80px)]">
    <div class="max-w-4xl mx-auto px-6">
        ...
        <form action="/admin/articles/{{ $article->id }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-8 rounded-[2rem] shadow-soft border border-secondary/50 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="category_label" value="{{ old('category_label', $article->category_label) }}" required
                               class="w-full px-5 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-cta font-medium">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="date" value="{{ old('date', $article->date) }}" required
                               class="w-full px-5 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-cta font-medium"
                ...
                <div>
                    ...
                    @if($article->image)
                        <div class="mb-3 flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                            <img src="{{ $article->image }}" alt="Current cover" class="w-14 h-14 object-cover rounded-lg flex-shrink-0">
                            <div>
                                <p class="text-xs font-semibold text-slate-500">Current cover</p>
                                <p class="text-xs text-slate-400 mt-0.5 truncate max-w-xs">Upload or paste a new URL below to replace it.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <label for="photo-upload"
                           class="flex flex-col items-center justify-center gap-2 w-full h-32 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 hover:border-cta/50 hover:bg-primary/5 cursor-pointer transition-all group">
                        <svg class="w-7 h-7 text-slate-300 group-hover:text-cta transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <span class="text-sm font-medium text-slate-400 group-hover:text-cta transition-colors" id="photo-label">Click to upload a new photo</span>
                        ...
                        <input id="photo-upload" type="file" name="photo" accept="image/*" class="hidden"
                               onchange="document.getElementById('photo-label').textContent = this.files[0]?.name ?? 'Click to upload a new photo'">
                    </label>
                ...
                <div class="text-xs opacity-50 bg-slate-50 px-4 py-3 rounded-xl border border-slate-200">
                    Slug: <code class="font-mono">{{ $article->slug }}</code> — changing the title does not change the slug once published.
                </div>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2rem] shadow-soft border border-secondary/50 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="3" required
                              class="w-full px-5 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-cta font-medium resize-none">{{ old('excerpt', $article->excerpt) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" rows="18" required
                              class="w-full px-5 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:border-cta font-medium text-sm leading-relaxed">{{ old('content', $plainContent) }}</textarea>
